...
'''
annonate_cmsearch_results_v3.py
...
##3) Now we compare the lists - from the .tab results, and from the larger genome file.
        ...
        for value in range(0, len(genomenameslist)): #this is correct
            ...
            hitstrand = str(strandlist[value])
            ...
            for location in indices:
                ...
                strand = str(db_genestrand[location])
                name = str(db_genename[location])


###AND   THE GENE IS ON THE SAME STRAND AS THE HIT!!!
                if intmiddle in range(intstart, intend) and hitstrand == strand: ##if a hit is on a gene and on the same strand
                    count = count +1
                ...
                if intmiddle in range(intstart, intend) and name == "IGR":  ###This is if the hit is in an IGR =
                    count = count + 1

                    gene.append(str(db_genename[location]))
                    genome.append(db_genomename[location])
                    gene_start.append(db_genestart[location])
                    gene_end.append(db_geneend[location])
                    symbol.append(db_genesymbol[location])
                ...
                if intmiddle in range(intstart, intend) and hitstrand != strand:  #if the hit is in a gene but on the opposite strand
                    count = count + 1

                    gene.append("opposite " + str(db_genename[location]))
                    genome.append(db_genomename[location])
                    gene_start.append(db_genestart[location])
                    gene_end.append(db_geneend[location])
                    symbol.append(db_genesymbol[location])
